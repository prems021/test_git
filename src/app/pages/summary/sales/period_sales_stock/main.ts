...
@Component({
  selector: 'app-summary-sales-period-stock',
  templateUrl: './main.html',
  styleUrls: ['./main.scss'],
})
export class Summary_sales_period_stock implements OnInit {


  submitted = false;
  sdate : any;
  edate : any;
  public post_mo : any = {sdate :"",edate :"",fy_id:"",com_id:null};
  Onload : number = 1;
  ...
  generate()
  {
    ...
    if(this.sdate == null)
    {
      alert('select date range properly')
        ...
        this.api.summary_sales_period_stock(this.post_mo).subscribe((data:any) => {
          this.get_res_(data);
        });
    }
  }
            ...
            for(var j = 0;j<data[i].hb7_invoice_details.length;j++)  
             {
                ...
                if(data[i].hb7_invoice_details[j].qty > 0)
                {
